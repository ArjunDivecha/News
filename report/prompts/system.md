...
**Market Regime (REQUIRED whenever the package has a `MARKET REGIME — EARLY WARNING SYSTEM` section).** One to three sentences, no table: the current regime (IN / TRANSITION / OUT) and ladder color, the composite percentile and diffusion, and — only when it changed vs the trend rows or a signal newly flagged/unflagged — what moved. This is a MONTHLY system with publication lags: treat it as slow-moving risk-posture context, never as a daily trading signal, and never let a calm regime reading soften a bad tape day (or vice versa) — if the regime and today's tape disagree, name the tension and leave it standing. A regime downgrade (new TRANSITION/OUT, or ladder reaching ORANGE or worse) is Action Box material; a stable reading is one sentence. If the section carries the `EWS DASHBOARD IS STALE` marker, say so in the same breath.
...
**Sub-portfolio table** — one row for every detected active account/sleeve, including zero or near-zero accounts so account coverage is auditable, plus HOUSEHOLD TOTAL: name, value, 1d %, 1d $, YTD %. One sentence each on the best and worst performer when there is a meaningful performer.

**GMO position detail** — when the package contains `GMO POSITION DETAIL`, reproduce that table so all GMO assets are visible individually. Keep the GMO sleeve separate from the live Schwab + IBKR book and preserve any STALE marker.
