...
# Find proxy groups using fast similarity-based grouping
print("\n[6/6] Clustering into proxy groups...")
print(f"  Using fast similarity-based grouping on {len(df_clean)} assets...")

# Use direct similarity-based grouping for speed
...
print(f"  Processing assets...", end='', flush=True)
for i in range(len(df_clean)):
    if i % 500 == 0:
    ...
    # Find all unassigned assets with high similarity to asset i
    similar_to_i = []
    for j in range(len(df_clean)):
        if j not in assigned and proxy_score[i, j] > threshold:
            similar_to_i.append(j)
    
    if len(similar_to_i) > 1:
        # Create a group with all similar assets
        for asset_idx in similar_to_i:
            assigned.add(asset_idx)
            proxy_groups.append(group_id)
        group_id += 1
    else:
        # Single asset group
        assigned.add(i)
...
clusters = np.array(proxy_groups)
