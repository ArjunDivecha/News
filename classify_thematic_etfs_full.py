...
for idx in range(len(df)):
    row = df.iloc[idx]
    if (idx + 1) % 50 == 0:
        print(f"Progress: {idx + 1}/231 - Saving checkpoint...")
        df.to_excel("Thematic ETFs Classified PROGRESS.xlsx", index=False)

    # Handle the odd column name formatting
...
df.to_excel("Thematic ETFs Classified.xlsx", index=False)
print(f"\n✓ THEMATIC ETFs CLASSIFICATION COMPLETE")
print(f"  File: Thematic ETFs Classified.xlsx")
